 ...
 * 
 */
public class BasicCodeGenerator extends AbstractTitaniumCodeGenerator implements
		CodeGenerator {

	public BasicCodeGenerator(Logger logger, ReportingService reportingSvc,
			boolean supportJSS) {
	 ...
	 */
	@Override
	public void generateCodeForModel(TitaniumUIBaseElement baseContainer,
		...
		try {
			...
			elementsStack.push(baseContainer);

			// define the Window we are working with

			String moduleName = getModuleName(baseContainer);

			generatedCodeLines.add("\tFUI." + moduleName + " ={};");
			...
			for (String varName : trackedVariables) {
				...
				generatedCodeLines.add(10, "\t" + fqdn + varName + " = null ;");

			}

			doReporting(platform, "BASIC_CODE_GEN");
	...
	protected void generateCode(TitaniumUIBaseElement element,
		...
		generatedCodeLines.add("\t" + fqdn + exposedCreationMethod
				+ " = function(){");
	...
	protected void createAttachFunction(TitaniumUIBaseElement element,
		...
		if (!handled
			...
			generatedCodeLines.add("\t" + fqdn + attachFunctionName
					+ " = function(){");
			...
			// TODO: is there a bug here? If there is no children there is no
			// createAttach function, should we leave it as empty instead of a
			// no function ?
			...
			if (children != null) {
				...
				for (TitaniumUIBaseElement child : children) {
					...
					// Why are we tracking twice ... Remove this. If this is an
					// Element , then its being tracked part of the Generate
					// Code op
					...
					generatedCodeLines.add("\t\t" + fqdn + parentVarName + "."
							+ childAdditionToParentMethod + "(" + fqdn
							+ childVarName + ");");
				...
				if (TitaniumUIBaseElement.class.isAssignableFrom(fieldType)) {
					...
					if (childElement != null) {
						...
						if (composite) {
							...
							// variable
							// local var
							generatedCodeLines.add("\t\t" + fqdn + methodName
									+ "();");

							// add its a property which should be added

							propWriter.keyUnescaped(fieldName).valueUnescaped(
									fqdn + localVariableName);
	...
	protected void handleBarSpecialProperties(TitaniumUIBaseElement element,
		...
		if (element instanceof TabbedBar || element instanceof ButtonBar) {
			...
			generatedCodeLines.add("\t\t" + fqdn + tabBarLabelsArrayName
					+ " = " + sw.toString() + ";");

			propWriter.keyUnescaped("labels").valueUnescaped(
					fqdn + tabBarLabelsArrayName);
	...
	protected void handleTableSpecialProperties(TitaniumUIBaseElement element,
		...
		if (element instanceof TableView) {
			...
			generatedCodeLines.add("\t\tif(" + fqdn + extensionMethodName
					+ "){");
			generatedCodeLines.add("\t\t\t" + fqdn + dataArrayName + " = "
					+ fqdn + extensionMethodName + "();");
			...
			for (int i = 0; i < tv.getChildren().size(); i++) {
				...
				generatedCodeLines.add("\t\t\t" + fqdn + createAttachMethod
						+ "();");
			...
			propWriter.keyUnescaped("data")
					.valueUnescaped(fqdn + dataArrayName);
	...
	protected void handleTabSpecialProperties(TitaniumUIBaseElement element, JSONWriter propWriter, String fqdn) throws JSONException {
		if(element instanceof Tab){
			String window = ((Tab)element).getWindow();
			propWriter.keyUnescaped("window");
			if(window != null && !window.isEmpty())
				propWriter.valueUnescaped(window);
			else
