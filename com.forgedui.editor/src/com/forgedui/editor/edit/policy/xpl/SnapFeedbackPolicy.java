 ...
 *******************************************************************************/
package com.forgedui.editor.edit.policy.xpl;
 ...
 * @author Pratik Shah
 * 
 * Updated by Dmitry {dmitry.grimm@gmail.com} - made all the guides in blue color
 * 
 * @since 3.0
 */
public class SnapFeedbackPolicy extends GraphicalEditPolicy {
	...
	void highlightGuide(Integer pos, Color color, int offset) {
		...
		boolean calcTitleBar = true;
		if(loc.x<0)
			calcTitleBar = false;
		int titleBar = getTitleBarHeight(getHost());
		...
		if (location[offset] == null || !location[offset].equals(pos)) {
			...
			if ((offset % 2) == 1) {
				figBounds.height = 1;
				if(!calcTitleBar)
					titleBar = -ScreenManager.CarrierHeight;
				figBounds.y = position - titleBar;
			} else {
	...
	private int getTitleBarHeight(EditPart host) {
		if(host instanceof WindowEditPart){
			List children = host.getChildren();
			for (Object child : children) {
				if(child instanceof TitleBarEditPart)
					return ((TitleBarEditPart)child).getBounds().height;
			}
		}else if(host != null)
			return getTitleBarHeight(host.getParent());
		return 0;
	}
